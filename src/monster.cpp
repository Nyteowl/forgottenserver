		...
		case TARGETSEARCH_NEAREST: {
			...
			if (!resultList.empty()) {
				...
				if (++it != resultList.end()) {
					...
					int32_t minRange = Position::getDistanceX(myPos, targetPosition) +
														 Position::getDistanceY(myPos, targetPosition);
					do {
						const Position& pos = (*it)->getPosition();

						int32_t distance =
								Position::getDistanceX(myPos, pos) + Position::getDistanceY(myPos, pos);
...
BlockType_t Monster::blockHit(Creature* attacker, CombatType_t combatType, int32_t& damage,
	...
	BlockType_t blockType =
			Creature::blockHit(attacker, combatType, damage, checkDefense, checkArmor);
...
bool Monster::canUseAttack(const Position& pos, const Creature* target) const {
	if (isHostile()) {
		...
		uint32_t distance = std::max<uint32_t>(Position::getDistanceX(pos, targetPos),
...
bool Monster::canUseSpell(const Position& pos, const Position& targetPos, const spellBlock_t& sb,
													uint32_t interval, bool& inRange, bool& resetTicks) {
	inRange = true;
...
bool Monster::pushCreature(Creature* creature) {
	static std::vector<Direction> dirList{DIRECTION_NORTH, DIRECTION_WEST, DIRECTION_EAST,
																				DIRECTION_SOUTH};
...
bool Monster::getDanceStep(const Position& creaturePos, Direction& direction,
	...
	if (!keepDistance || offset_y >= 0) {
		uint32_t tmpDist =
				std::max<uint32_t>(distance_x, std::abs((creaturePos.getY() - 1) - centerPos.getY()));
		if (tmpDist == centerToDist && canWalkTo(creaturePos, DIRECTION_NORTH)) {
			bool result = true;

			if (keepAttack) {
				result = (!canDoAttackNow ||
									canUseAttack(Position(creaturePos.x, creaturePos.y - 1, creaturePos.z),
															 attackedCreature));
			}
	...
	if (!keepDistance || offset_x <= 0) {
		uint32_t tmpDist =
				std::max<uint32_t>(std::abs((creaturePos.getX() + 1) - centerPos.getX()), distance_y);
		if (tmpDist == centerToDist && canWalkTo(creaturePos, DIRECTION_EAST)) {
			bool result = true;

			if (keepAttack) {
				result = (!canDoAttackNow ||
									canUseAttack(Position(creaturePos.x + 1, creaturePos.y, creaturePos.z),
															 attackedCreature));
			}
...
bool Monster::getDistanceStep(const Position& targetPos, Direction& direction,
	...
	if (!flee && (distance > mType->info.targetDistance ||
		...
		return true;	// we don't really care here, since it's what we wanted to reach (a dancestep will
									// take of dancing in that position)
	}
	...
	if (offsetx == 0 && offsety == 0) {
		return getRandomStep(creaturePos, direction);	// player is "on" the monster so let's get some
																									 // random step and rest will be taken care later.
	}
	...
	// Now let's decide where the player is located to the monster (what direction) so we can decide
	// where to escape.
	if (dy > dx) {
		Direction playerDir = offsety < 0 ? DIRECTION_SOUTH : DIRECTION_NORTH;
		switch (playerDir) {
			case DIRECTION_NORTH: {
				// Player is to the NORTH, so obviously we need to check if we can go SOUTH, if not then
				// let's choose WEST or EAST and again if we can't we need to decide about some diagonal
				// movements.
