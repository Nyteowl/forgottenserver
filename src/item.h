...
class ItemAttributes {
 ...
 private:
	...
	void setCustomAttribute(std::string& key, CustomAttribute& value) {
		...
		getAttr(ITEM_ATTRIBUTE_CUSTOM)
				.value.custom->insert(std::make_pair(std::move(key), std::move(value)));
	}
	...
	const static uint32_t intAttributeTypes =
			ITEM_ATTRIBUTE_ACTIONID | ITEM_ATTRIBUTE_UNIQUEID | ITEM_ATTRIBUTE_DATE |
	...
	const static uint32_t stringAttributeTypes = ITEM_ATTRIBUTE_DESCRIPTION | ITEM_ATTRIBUTE_TEXT |
																							 ITEM_ATTRIBUTE_WRITER | ITEM_ATTRIBUTE_NAME |
																							 ITEM_ATTRIBUTE_ARTICLE | ITEM_ATTRIBUTE_PLURALNAME;
...
class Item : virtual public Thing {
 public:
	...
	static std::string getNameDescription(const ItemType& it, const Item* item = nullptr,
	...
	bool isCleanable() const {
		return !loadedFromMap && canRemove() && isPickupable() &&
					 !hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) && !hasAttribute(ITEM_ATTRIBUTE_ACTIONID);
	}
