...
bool Weapons::registerEvent(Event_ptr event, const pugi::xml_node&) {
	...
	if (!result.second) {
		std::cout << "[Warning - Weapons::registerEvent] Duplicate registered item with id: "
							<< weapon->getID() << std::endl;
	}
...
int32_t Weapons::getMaxMeleeDamage(int32_t attackSkill, int32_t attackValue) {
	return static_cast<int32_t>(
			std::ceil((attackSkill * (attackValue * 0.05)) + (attackValue * 0.5)));
}

// players
int32_t Weapons::getMaxWeaponDamage(uint32_t level, int32_t attackSkill, int32_t attackValue,
																		float attackFactor) {
	return static_cast<int32_t>(std::round(
			(level / 5) + (((((attackSkill / 4.) + 1) * (attackValue / 3.)) * 1.03) / attackFactor)));
}
...
bool Weapon::configureEvent(const pugi::xml_node& node) {
	...
	if ((attr = node.attribute("action"))) {
		...
		if (action == WEAPONACTION_NONE) {
			std::cout << "[Warning - Weapon::configureEvent] Unknown action " << attr.as_string()
								<< std::endl;
		}
	}
...
int32_t Weapon::playerWeaponCheck(Player* player, Creature* target, uint8_t shootRange) const {
	...
	if (std::max<uint32_t>(Position::getDistanceX(playerPos, targetPos),
												 Position::getDistanceY(playerPos, targetPos)) > shootRange) {
		return 0;
	}
...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target,
...
void Weapon::onUsedWeapon(Player* player, Item* item, Tile* destTile) const {
	...
	switch (action) {
		...
		case WEAPONACTION_MOVE:
			g_game.internalMoveItem(item->getParent(), destTile, INDEX_WHEREEVER, item, 1, nullptr,
															FLAG_NOLIMIT);
			break;
...
bool WeaponMelee::getSkillType(const Player* player, const Item* item, skills_t& skill,
															 uint32_t& skillpoint) const {
...
int32_t WeaponMelee::getElementDamage(const Player* player, const Creature*,
...
bool WeaponDistance::useWeapon(Player* player, Item* item, Creature* target) const {
	...
	if (it.weaponType == WEAPON_AMMO) {
		...
		if (mainWeapon) {
			damageModifier =
					mainWeapon->playerWeaponCheck(player, target, mainWeaponItem->getShootRange());
	...
	if (chance >= uniform_random(1, 100)) {
		...
		if (!Position::areInRange<1, 1, 0>(player->getPosition(), target->getPosition())) {
			...
			for (const auto& dir : destList) {
				...
				if (tmpTile && !tmpTile->hasFlag(TILESTATE_IMMOVABLEBLOCKSOLID) &&
						tmpTile->getGround() != nullptr) {
					destTile = tmpTile;
					break;
				}
			}
...
int32_t WeaponDistance::getElementDamage(const Player* player, const Creature* target,
																				 const Item* item) const {
	if (elementType == COMBAT_NONE) {
		return 0;
	}
...
int32_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target,
...
bool WeaponDistance::getSkillType(const Player* player, const Item*, skills_t& skill,
																	uint32_t& skillpoint) const {
...
bool WeaponWand::configureEvent(const pugi::xml_node& node) {
	...
	if (tmpStrValue == "earth") {
		...
		std::cout << "[Warning - WeaponWand::configureEvent] Type \"" << attr.as_string()
...
int32_t WeaponWand::getWeaponDamage(const Player*, const Creature*, const Item*,
																		bool maxDamage /*= false*/) const {
	if (maxDamage) {
		return -maxChange;
	}
