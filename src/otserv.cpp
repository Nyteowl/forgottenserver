...
[[noreturn]] void badAllocationHandler() {
	...
	puts(
			"Allocation failed, server out of memory.\nDecrease the size of your map or compile in 64 "
			"bits mode.\n");
	getchar();
	exit(-1);
}
...
void printServerVersion() {
	...
	std::cout << "Visit our forum for updates, support, and resources: https://otland.net/."
						<< std::endl;
	std::cout << std::endl;
}
...
void mainLoader(int, char* [], ServiceManager* services) {
	...
	if (!DatabaseManager::isDatabaseSetup()) {
		startupErrorMessage(
				"The database you have specified in config.lua is empty, please import the schema.sql to "
				"your database.");
		return;
	}
	...
	services->add<ProtocolLogin>(
			static_cast<uint16_t>(g_config.getNumber(ConfigManager::LOGIN_PORT)));

	// OT protocols
	services->add<ProtocolStatus>(
			static_cast<uint16_t>(g_config.getNumber(ConfigManager::STATUS_PORT)));
	...
	std::string strRentPeriod =
			asLowerCaseString(g_config.getString(ConfigManager::HOUSE_RENT_PERIOD));
	...
	if (getuid() == 0 || geteuid() == 0) {
		std::cout << "> Warning: " << STATUS_SERVER_NAME
							<< " has been executed as root user, please consider running it as a normal user."
							<< std::endl;
	}
#endif
