...
bool CreatureEvents::registerEvent(Event_ptr event, const pugi::xml_node&) {
	...
	if (creatureEvent->getEventType() == CREATURE_EVENT_NONE) {
		std::cout << "Error: [CreatureEvents::registerEvent] Trying to register event without type!"
							<< std::endl;
		return false;
	}
...
bool CreatureEvents::registerLuaEvent(CreatureEvent* event) {
	...
	if (creatureEvent->getEventType() == CREATURE_EVENT_NONE) {
		std::cout << "Error: [CreatureEvents::registerLuaEvent] Trying to register event without type!"
							<< std::endl;
		return false;
	}
...
bool CreatureEvent::configureEvent(const pugi::xml_node& node) {
	...
	if (!nameAttribute) {
		std::cout << "[Error - CreatureEvent::configureEvent] Missing name for creature event"
							<< std::endl;
		return false;
	}
	...
	if (!typeAttribute) {
		std::cout << "[Error - CreatureEvent::configureEvent] Missing type for creature event: "
							<< eventName << std::endl;
		return false;
	}
	...
	if (tmpStr == "login") {
		...
		std::cout << "[Error - CreatureEvent::configureEvent] Invalid type for creature event: "
							<< eventName << std::endl;
		return false;
	}
...
bool CreatureEvent::executeOnDeath(Creature* creature, Item* corpse, Creature* killer,
...
void CreatureEvent::executeModalWindow(Player* player, uint32_t modalWindowId, uint8_t buttonId,
																			 uint8_t choiceId) {
...
void CreatureEvent::executeHealthChange(Creature* creature, Creature* attacker,
																				CombatDamage& damage) {
	// onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType,
	// origin)
	if (!scriptInterface->reserveScriptEnv()) {
...
void CreatureEvent::executeManaChange(Creature* creature, Creature* attacker,
																			CombatDamage& damage) {
	// onManaChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType,
	// origin)
	if (!scriptInterface->reserveScriptEnv()) {
...
void CreatureEvent::executeExtendedOpcode(Player* player, uint8_t opcode,
