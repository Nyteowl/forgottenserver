...
class Database {
 ...
 private:
	 ...
	 *
	 * Methods for starting, commiting and rolling back transaction. Each of the returns boolean
	 * value.
	 *
	 * @return true on success, false on error
	 */
...
class DBResult {
 public:
	...
	T getNumber(const std::string& s) const {
		...
		if (it == listNames.end()) {
			std::cout << "[Error - DBResult::getNumber] Column '" << s
								<< "' doesn't exist in the result set" << std::endl;
