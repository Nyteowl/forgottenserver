...
void PrivateChatChannel::invitePlayer(const Player& player, Player& invitePlayer) {
	...
	ss << player.getName() << " invites you to "
		 << (player.getSex() == PLAYERSEX_FEMALE ? "her" : "his") << " private chat channel.";
	...
	if (id == CHANNEL_GUILD) {
		...
		if (guild && !guild->getMotd().empty()) {
			g_scheduler.addEvent(
					createSchedulerTask(150, std::bind(&Game::sendGuildMotd, &g_game, player.getID())));
		}
	}
...
bool ChatChannel::executeOnSpeakEvent(const Player& player, SpeakClasses& type,
...
Chat::Chat()
		: scriptInterface("Chat Interface"), dummyPrivate(CHANNEL_PRIVATE, "Private Chat Channel") {
...
bool Chat::load() {
	...
	for (auto channelNode : doc.child("channels").children()) {
		...
		if (it != normalChannels.end()) {
			...
			if (scriptAttribute) {
				if (scriptInterface.loadFile("data/chatchannels/scripts/" +
																		 std::string(scriptAttribute.as_string())) == 0) {
	...
	switch (channelId) {
		case CHANNEL_GUILD: {
			...
			if (guild) {
				auto ret = guildChannels.emplace(
						std::make_pair(guild->getId(), ChatChannel(channelId, guild->getName())));
		...
		case CHANNEL_PRIVATE: {
			...
			for (uint16_t i = 100; i < 10000; ++i) {
				auto ret = privateChannels.emplace(
						std::make_pair(i, PrivateChatChannel(i, player.getName() + "'s Channel")));
				if (ret.second) {	// second is a bool that indicates that a new channel has been placed in
													 // the map
...
bool Chat::talkToChannel(const Player& player, SpeakClasses type, const std::string& text,
												 uint16_t channelId) {
	ChatChannel* channel = getChannel(player, channelId);
	if (!channel) {
		return false;
