...
class Player final : public Creature, public Cylinder {
 public:
	...
	void addBlessing(uint8_t blessing) { blessings |= blessing; }
	void removeBlessing(uint8_t blessing) { blessings &= ~blessing; }
	...
	bool removeItemOfType(uint16_t itemId, uint32_t amount, int32_t subType,
	...
	void sendChannelEvent(uint16_t channelId, const std::string& playerName,
	...
	void sendCreatureAppear(const Creature* creature, const Position& pos, bool isLogin) {
		if (client) {
			client->sendAddCreature(creature, pos,
															creature->getTile()->getStackposOfCreature(this, creature), isLogin);
		}
	}
	void sendCreatureMove(const Creature* creature, const Position& newPos, int32_t newStackPos,
	...
	void sendToChannel(const Creature* creature, SpeakClasses type, const std::string& text,
 ...
 private:
	...
	std::map<uint32_t, uint32_t>& getAllItemTypeCount(
			std::map<uint32_t, uint32_t>& countMap) const override;
	...
	std::forward_list<Condition*> storedConditionList;	// TODO: This variable is only temporarily
																											// used when logging in, get rid of it somehow
