	...
	if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		if (!player->hasFlag(PlayerFlag_CannotBeBanned)) {
			...
			if (IOBan::isAccountBanned(accountId, banInfo)) {
				...
				if (banInfo.expiresAt > 0) {
					ss << "Your account has been banned until " << formatDateShort(banInfo.expiresAt)
						 << " by " << banInfo.bannedBy << ".\n\nReason specified:\n" << banInfo.reason;
				} else {
		...
		if (foundPlayer->client) {
			...
			eventConnect = g_scheduler.addEvent(createSchedulerTask(
					1000,
					std::bind(&ProtocolGame::connect, getThis(), foundPlayer->getID(), operatingSystem)));
		} else {
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg) {
	...
	g_dispatcher.addTask(createTask(
			std::bind(&ProtocolGame::login, getThis(), characterName, accountId, operatingSystem)));
}
...
void ProtocolGame::parsePacket(NetworkMessage& msg) {
	...
	switch (recvbyte) {
		...
		case 0x6D:
			...
			break;
		case 0x6F:
			addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerTurn, player->getID(),
											 DIRECTION_NORTH);
			break;
		case 0x70:
			addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerTurn, player->getID(),
		...
		default:
			// std::cout << "Player: " << player->getName() << " sent an unknown packet header: 0x" <<
...
void ProtocolGame::GetFloorDescription(NetworkMessage& msg, int32_t x, int32_t y, int32_t z,
																			 int32_t width, int32_t height, int32_t offset,
																			 int32_t& skip) {
...
void ProtocolGame::parseUseItem(NetworkMessage& msg) {
	...
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseItem, player->getID(), pos, stackpos,
									 index, spriteId);
}
...
void ProtocolGame::parseUseItemEx(NetworkMessage& msg) {
	...
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseItemEx, player->getID(), fromPos,
									 fromStackPos, fromSpriteId, toPos, toStackPos, toSpriteId);
}
...
void ProtocolGame::parseUseWithCreature(NetworkMessage& msg) {
	...
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseWithCreature, player->getID(),
									 fromPos, fromStackPos, creatureId, spriteId);
}
...
void ProtocolGame::parseThrow(NetworkMessage& msg) {
	...
	if (toPos != fromPos) {
		addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerMoveThing, player->getID(), fromPos,
										 spriteId, fromStackpos, toPos, count);
	}
}
...
void ProtocolGame::parseLookInBattleList(NetworkMessage& msg) {
	uint32_t creatureId = msg.get<uint32_t>();
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerLookInBattleList, player->getID(),
									 creatureId);
}
...
void ProtocolGame::parseWrapItem(NetworkMessage& msg) {
	...
	uint16_t spriteId = msg.get<uint16_t>();
	uint8_t stackpos = msg.getByte();
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerWrapItem, player->getID(), pos,
									 stackpos, spriteId);
}
...
void ProtocolGame::parsePlayerPurchase(NetworkMessage& msg) {
	...
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerPurchaseItem, player->getID(), id,
									 count, amount, ignoreCap, inBackpacks);
}
...
void ProtocolGame::parsePlayerSale(NetworkMessage& msg) {
	...
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerSellItem, player->getID(), id, count,
...
void ProtocolGame::parseLookInTrade(NetworkMessage& msg) {
	...
	uint8_t index = msg.getByte();
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerLookInTrade, player->getID(),
									 counterOffer, index);
}
...
void ProtocolGame::parseRotateItem(NetworkMessage& msg) {
	...
	uint16_t spriteId = msg.get<uint16_t>();
	uint8_t stackpos = msg.getByte();
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerRotateItem, player->getID(), pos,
									 stackpos, spriteId);
}
...
void ProtocolGame::parseRuleViolationReport(NetworkMessage& msg) {
	...
	addGameTask(&Game::playerReportRuleViolation, player->getID(), targetName, reportType,
...
void ProtocolGame::parseMarketLeave() { addGameTask(&Game::playerLeaveMarket, player->getID()); }
...
void ProtocolGame::parseMarketCreateOffer(NetworkMessage& msg) {
	...
	addGameTask(&Game::playerCreateMarketOffer, player->getID(), type, spriteId, amount, price,
							anonymous);
}
...
void ProtocolGame::sendChannelEvent(uint16_t channelId, const std::string& playerName,
																		ChannelEvent_t channelEvent) {
	NetworkMessage msg;
	msg.addByte(0xF3);
...
void ProtocolGame::sendChannelMessage(const std::string& author, const std::string& text,
																			SpeakClasses type, uint16_t channel) {
	NetworkMessage msg;
	msg.addByte(0xAA);
...
void ProtocolGame::sendContainer(uint8_t cid, const Container* container, bool hasParent,
	...
	if (firstIndex < containerSize) {
		uint8_t itemsToSend =
				std::min<uint32_t>(std::min<uint32_t>(container->capacity(), containerSize - firstIndex),
													 std::numeric_limits<uint8_t>::max());

		msg.addByte(itemsToSend);
		for (auto it = container->getItemList().begin() + firstIndex, end = it + itemsToSend; it != end;
				 ++it) {
			msg.addItem(*it);
		}
	} else {
...
void ProtocolGame::sendSaleItemList(const std::list<ShopInfo>& shop) {
	...
	uint8_t i = 0;
	for (std::map<uint16_t, uint32_t>::const_iterator it = saleMap.begin(); i < itemsToSend;
			 ++it, ++i) {
...
void ProtocolGame::sendMarketEnter(uint32_t depotId) {
	...
	msg.addByte(std::min<uint32_t>(IOMarket::getPlayerOfferCount(player->getGUID()),
																 std::numeric_limits<uint8_t>::max()));
	...
	uint16_t i = 0;
	for (std::map<uint16_t, uint32_t>::const_iterator it = depotItems.begin(); i < itemsToSend;
			 ++it, ++i) {
...
void ProtocolGame::sendMarketBrowseOwnOffers(const MarketOfferList& buyOffers,
																						 const MarketOfferList& sellOffers) {
	NetworkMessage msg;
	msg.addByte(0xF9);
...
void ProtocolGame::sendMarketDetail(uint16_t itemId) {
	...
	if (it.attack != 0) {
		...
		if (it.abilities && it.abilities->elementType != COMBAT_NONE &&
				it.abilities->elementDamage != 0) {
			std::ostringstream ss;
			ss << it.attack << " physical +" << it.abilities->elementDamage << ' '
				 << getCombatName(it.abilities->elementType);
	...
	if (it.abilities) {
		...
		for (size_t i = 0; i < COMBAT_COUNT; ++i) {
			...
			ss << getCombatName(indexToCombatType(i)) << ' ' << std::showpos
				 << it.abilities->absorbPercent[i] << std::noshowpos << '%';
		}
	...
	if (it.abilities) {
		...
		if (it.abilities->stats[STAT_MAGICPOINTS] != 0) {
			...
			ss << "magic level " << std::showpos << it.abilities->stats[STAT_MAGICPOINTS]
				 << std::noshowpos;
		}
	...
	if (statistics) {
		...
		msg.add<uint32_t>(
				std::min<uint64_t>(std::numeric_limits<uint32_t>::max(), statistics->totalPrice));
...
void ProtocolGame::sendCreatureSay(const Creature* creature, SpeakClasses type,
...
void ProtocolGame::sendToChannel(const Creature* creature, SpeakClasses type,
...
void ProtocolGame::sendPrivateMessage(const Player* speaker, SpeakClasses type,
...
void ProtocolGame::sendCreatureHealth(const Creature* creature) {
	...
	if (creature->isHealthHidden()) {
		...
		msg.addByte(std::ceil((static_cast<double>(creature->getHealth()) /
...
void ProtocolGame::sendMoveCreature(const Creature* creature, const Position& newPos,
																		int32_t newStackPos, const Position& oldPos,
...
void ProtocolGame::sendTextWindow(uint32_t windowTextId, Item* item, uint16_t maxlen,
																	bool canWrite) {
	NetworkMessage msg;
	msg.addByte(0x96);
...
void ProtocolGame::sendOutfitWindow() {
	...
	for (const Outfit& outfit : outfits) {
		...
		if (protocolOutfits.size() == std::numeric_limits<uint8_t>::max()) {	// Game client currently
...
void ProtocolGame::sendVIP(uint32_t guid, const std::string& name, const std::string& description,
													 uint32_t icon, bool notify, VipStatus_t status) {
	NetworkMessage msg;
	msg.addByte(0xD2);
	msg.add<uint32_t>(guid);
	msg.addString(name);
...
////////////// Add common messages
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known,
															 uint32_t remove) {
...
void ProtocolGame::AddPlayerStats(NetworkMessage& msg) {
	...
	msg.add<uint16_t>(
			std::min<int32_t>(player->getMaxHealth(), std::numeric_limits<uint16_t>::max()));
...
void ProtocolGame::AddPlayerSkills(NetworkMessage& msg) {
	...
	for (uint8_t i = SKILL_FIRST; i <= SKILL_LAST; ++i) {
		msg.add<uint16_t>(
				std::min<int32_t>(player->getSkillLevel(i), std::numeric_limits<uint16_t>::max()));
