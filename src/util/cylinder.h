...
enum cylinderflags_t {
	FLAG_NOLIMIT =
			1 << 0,	// Bypass limits like capacity/container limits, blocking items/creatures etc.
...
class Cylinder : virtual public Thing {
 public:
	...
	/**
		...
		* \returns ReturnValue holds the return value
		*/
	virtual ReturnValue queryRemove(const Thing& thing, uint32_t count, uint32_t flags,
																	Creature* = nullptr) const = 0;
	...
	/**
		...
		*/
	virtual std::map<uint32_t, uint32_t>& getAllItemTypeCount(
...
class VirtualCylinder final : public Cylinder {
 public:
	...
	virtual ReturnValue queryAdd(int32_t, const Thing&, uint32_t, uint32_t,
															 Creature* = nullptr) const override {
		return RETURNVALUE_NOTPOSSIBLE;
	}
	virtual ReturnValue queryMaxCount(int32_t, const Thing&, uint32_t, uint32_t&,
																		uint32_t) const override {
		return RETURNVALUE_NOTPOSSIBLE;
	}
	virtual ReturnValue queryRemove(const Thing&, uint32_t, uint32_t,
																	Creature* = nullptr) const override {
		return RETURNVALUE_NOTPOSSIBLE;
	}
	...
	virtual void postAddNotification(Thing*, const Cylinder*, int32_t,
																	 cylinderlink_t = LINK_OWNER) override {}
	virtual void postRemoveNotification(Thing*, const Cylinder*, int32_t,
																			cylinderlink_t = LINK_OWNER) override {}
