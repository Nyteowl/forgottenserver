...
Condition* Condition::createCondition(ConditionId_t id, ConditionType_t type, int32_t ticks,
...
Condition* Condition::createCondition(PropStream& propStream) {
	...
	return createCondition(static_cast<ConditionId_t>(id), static_cast<ConditionType_t>(type), ticks,
												 0, buff != 0, subId, aggressive);
}
	...
	for (int32_t i = STAT_FIRST; i <= STAT_LAST; ++i) {
		...
		switch (i) {
			...
			case STAT_MAGICPOINTS:
				stats[i] =
						static_cast<int32_t>(player->getBaseMagicLevel() * ((statsPercent[i] - 100) / 100.f));
				break;
		}
	}
}
...
void ConditionRegeneration::addCondition(Creature*, const Condition* condition) {
	if (updateCondition(condition)) {
		...
		const ConditionRegeneration& conditionRegen =
				static_cast<const ConditionRegeneration&>(*condition);
