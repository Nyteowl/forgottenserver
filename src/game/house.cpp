...
void House::setOwner(uint32_t guid, bool updateDatabase /* = true*/,
	...
	if (updateDatabase && owner != guid) {
		...
		query << "UPDATE `houses` SET `owner` = " << guid
					<< ", `bid` = 0, `bid_end` = 0, `last_bid` = 0, `highest_bidder` = 0  WHERE `id` = "
					<< id;
...
bool House::kickPlayer(Player* player, Player* target) {
	...
	if (getHouseAccessLevel(player) < getHouseAccessLevel(target) ||
			target->hasFlag(PlayerFlag_CanEditHouses)) {
		return false;
	}
...
bool House::transferToDepot(Player* player) const {
	...
	for (Item* item : moveItemList) {
		g_game.internalMoveItem(item->getParent(), player->getInbox(), INDEX_WHEREEVER, item,
		...
		if (player.getBankBalance() >= rent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				ss << "Warning! \nThe " << period << " rent of " << house->getRent()
					 ...
					 << daysLeft << " days or you will lose this house.";
