...
class Weapons final : public BaseEvents {
 public:
	...
	static int32_t getMaxWeaponDamage(uint32_t level, int32_t attackSkill, int32_t attackValue,
																		float attackFactor);
...
class Weapon : public Event {
 public:
	...
	virtual int32_t getElementDamage(const Player* player, const Creature* target,
 ...
 protected:
	void internalUseWeapon(Player* player, Item* item, Creature* target,
...
class WeaponMelee final : public Weapon {
 public:
	...
	int32_t getElementDamage(const Player* player, const Creature* target,
													 const Item* item) const override;
	CombatType_t getElementType() const override { return elementType; }

 private:
	bool getSkillType(const Player* player, const Item* item, skills_t& skill,
