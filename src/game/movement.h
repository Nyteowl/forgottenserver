...
using EquipFunction = std::function<ReturnValue(MoveEvent* moveEvent, Player* player, Item* item,
																								slots_t slot, bool boolean)>;
...
class MoveEvent final : public Event {
 public:
	...
	std::vector<uint32_t> getItemIdRange() { return itemIdRange; }
	void addItemId(uint32_t id) { itemIdRange.emplace_back(id); }
	std::vector<uint32_t> getActionIdRange() { return actionIdRange; }
	void addActionId(uint32_t id) { actionIdRange.emplace_back(id); }
	std::vector<uint32_t> getUniqueIdRange() { return uniqueIdRange; }
	void addUniqueId(uint32_t id) { uniqueIdRange.emplace_back(id); }
	std::vector<Position> getPosList() { return posList; }
	...
	static ReturnValue EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot,
															 bool isCheck);
	static ReturnValue DeEquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot,
																 bool);
