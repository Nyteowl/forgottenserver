...
ConditionDamage* Monsters::getDamageCondition(ConditionType_t conditionType, int32_t maxDamage,
...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb,
	...
	if (isScripted) {
		...
		std::unique_ptr<CombatSpell> combatSpellPtr(
				new CombatSpell(nullptr, needTarget, needDirection));
		if (!combatSpellPtr->loadScript("data/" + g_spells->getScriptBaseName() + "/scripts/" +
																		scriptName)) {
			return false;
		}
		...
		combatSpell->getCombat()->setPlayerCombatValues(COMBAT_FORMULA_DAMAGE, sb.minCombatValue, 0,
																										sb.maxCombatValue, 0);
	} else {
		...
		if (tmpName == "melee") {
			...
			if ((attackAttribute = node.attribute("attack")) &&
					(skillAttribute = node.attribute("skill"))) {
				sb.minCombatValue = 0;
				sb.maxCombatValue =
						-Weapons::getMaxMeleeDamage(pugi::cast<int32_t>(skillAttribute.value()),
			...
			if (conditionType != CONDITION_NONE) {
				Condition* condition =
						getDamageCondition(conditionType, maxDamage, minDamage, 0, tickInterval);
		...
		combat->setPlayerCombatValues(COMBAT_FORMULA_DAMAGE, sb.minCombatValue, 0, sb.maxCombatValue,
																	0);
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb,
																const std::string& description) {
	...
	if (spell->isScripted) {
		std::unique_ptr<CombatSpell> combatSpellPtr(
				new CombatSpell(nullptr, spell->needTarget, spell->needDirection));
		if (!combatSpellPtr->loadScript("data/" + g_spells->getScriptBaseName() + "/scripts/" +
																		spell->scriptName)) {
		...
		if (tmpName == "melee") {
			...
			Condition* condition =
					getDamageCondition(spell->conditionType, maxDamage, minDamage, startDamage, tickInterval);
			combat->addCondition(condition);
		} else if (tmpName == "strength") {
			//
		} else if (tmpName == "effect") {
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName,
	...
	if ((attr = monsterNode.attribute("race"))) {
		...
		if (tmpStrValue == "venom" || tmpInt == 1) {
			...
			std::cout << "[Warning - Monsters::loadMonster] Unknown race type " << attr.as_string()
								<< ". " << file << std::endl;
		}
	}
	...
	if ((node = monsterNode.child("flags"))) {
		for (auto flagNode : node.children()) {
			...
			if (strcasecmp(attrName, "summonable") == 0) {
				...
				if (targetDistance < 1) {
					targetDistance = 1;
					std::cout << "[Warning - Monsters::loadMonster] targetdistance less than 1. " << file
										<< std::endl;
				}
				...
				std::cout << "[Warning - Monsters::loadMonster] Unknown flag attribute: " << attrName
									<< ". " << file << std::endl;
			}
		}
	...
	if (mType->info.manaCost == 0 && (mType->info.isSummonable || mType->info.isConvinceable)) {
		std::cout << "[Warning - Monsters::loadMonster] manaCost missing or zero on monster with "
								 "summonable and/or convinceable flags: "
							<< file << std::endl;
	}
	...
	if ((node = monsterNode.child("targetchange"))) {
		if ((attr = node.attribute("speed")) || (attr = node.attribute("interval"))) {
			...
			std::cout << "[Warning - Monsters::loadMonster] Missing targetchange speed. " << file
								<< std::endl;
		}
		...
		if ((attr = node.attribute("chance"))) {
			...
			if (chance > 100) {
				chance = 100;
				std::cout << "[Warning - Monsters::loadMonster] targetchange chance value out of bounds. "
									<< file << std::endl;
			}
			mType->info.changeTargetChance = chance;
		} else {
			std::cout << "[Warning - Monsters::loadMonster] Missing targetchange chance. " << file
								<< std::endl;
		}
	}
	...
	if ((node = monsterNode.child("look"))) {
		if ((attr = node.attribute("type"))) {
			...
			std::cout << "[Warning - Monsters::loadMonster] Missing look type/typeex. " << file
								<< std::endl;
		}
		...
		for (auto immunityNode : node.children()) {
			if ((attr = immunityNode.attribute("name"))) {
				...
				if (tmpStrValue == "physical") {
					...
					std::cout << "[Warning - Monsters::loadMonster] Unknown immunity name "
										<< attr.as_string() << ". " << file << std::endl;
				}
				...
				if (attr.as_bool()) {
					...
					mType->info.conditionImmunities |= CONDITION_FIRE;
								 ...
								 (attr = immunityNode.attribute("earth"))) {
				if (attr.as_bool()) {
					mType->info.damageImmunities |= COMBAT_EARTHDAMAGE;
					mType->info.conditionImmunities |= CONDITION_POISON;
				}
	...
	if ((node = monsterNode.child("voices"))) {
		...
		if ((attr = node.attribute("chance"))) {
			...
			if (chance > 100) {
				chance = 100;
				std::cout << "[Warning - Monsters::loadMonster] yell chance value out of bounds. " << file
									<< std::endl;
			}
		...
		for (auto voiceNode : node.children()) {
			...
			if ((attr = voiceNode.attribute("sentence"))) {
				...
				std::cout << "[Warning - Monsters::loadMonster] Missing voice sentence. " << file
									<< std::endl;
			}
		...
		for (auto elementNode : node.children()) {
			if ((attr = elementNode.attribute("physicalPercent"))) {
				...
				if (mType->info.damageImmunities & COMBAT_PHYSICALDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"physical\" on immunity "
											 "and element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_ICEDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"ice\" on immunity and "
											 "element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_EARTHDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"earth\" on immunity and "
											 "element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_FIREDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"fire\" on immunity and "
											 "element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_ENERGYDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"energy\" on immunity and "
											 "element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_HOLYDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"holy\" on immunity and "
											 "element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_DEATHDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"death\" on immunity and "
											 "element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_DROWNDAMAGE) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"drown\" on immunity and "
											 "element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_LIFEDRAIN) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"lifedrain\" on immunity "
											 "and element tags. "
										<< file << std::endl;
				}
				...
				if (mType->info.damageImmunities & COMBAT_MANADRAIN) {
					std::cout << "[Warning - Monsters::loadMonster] Same element \"manadrain\" on immunity "
											 "and element tags. "
										<< file << std::endl;
				}
			} else {
				std::cout << "[Warning - Monsters::loadMonster] Unknown element percent. " << file
									<< std::endl;
			}
		}
	}
	...
	if ((node = monsterNode.child("summons"))) {
		if ((attr = node.attribute("maxSummons"))) {
			...
			std::cout << "[Warning - Monsters::loadMonster] Missing summons maxSummons. " << file
								<< std::endl;
		}
		...
		for (auto summonNode : node.children()) {
			...
			if ((attr = summonNode.attribute("chance"))) {
				...
				if (chance > 100) {
					chance = 100;
					std::cout << "[Warning - Monsters::loadMonster] Summon chance value out of bounds. "
										<< file << std::endl;
				}
			}
	...
	if ((node = monsterNode.child("script"))) {
		for (auto eventNode : node.children()) {
			if ((attr = eventNode.attribute("name"))) {
				...
				std::cout << "[Warning - Monsters::loadMonster] Missing name for script event. " << file
									<< std::endl;
			}
		}
	}
...
bool Monsters::loadLootItem(const pugi::xml_node& node, LootBlock& lootBlock) {
	...
	if ((attr = node.attribute("id"))) {
		...
		if (it.name.empty()) {
			std::cout << "[Warning - Monsters::loadMonster] Unknown loot item id \"" << id << "\". "
								<< std::endl;
			return false;
		}
		...
		if (ids.first == Item::items.nameToItems.cend()) {
			std::cout << "[Warning - Monsters::loadMonster] Unknown loot item \"" << name << "\". "
								<< std::endl;
			return false;
		}
		...
		if (std::next(ids.first) != ids.second) {
			std::cout << "[Warning - Monsters::loadMonster] Non-unique loot item \"" << name << "\". "
								<< std::endl;
			return false;
		}
